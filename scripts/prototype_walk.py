...
# Discover Meshy Smart Rig roles structurally rather than by Bone_### names.
...
root=roots[0]; pelvis=root.children[0]
pchildren=list(pelvis.children)
spine_root=min(pchildren,key=lambda b:abs(b.head_local.x))
leg_roots=sorted([b for b in pchildren if b!=spine_root],key=lambda b:b.head_local.x)
if len(leg_roots)!=2: raise RuntimeError(f'Expected two leg roots, found {len(leg_roots)}')
left_leg,right_leg=leg_roots
spine=[spine_root]
while len(spine[-1].children)==1: spine.append(spine[-1].children[0])
chest=spine[-1]
jchildren=list(chest.children)
head=min(jchildren,key=lambda b:abs(b.head_local.x))
arms=sorted([b for b in jchildren if b!=head],key=lambda b:b.head_local.x)
if len(arms)<2: raise RuntimeError('Could not identify arms')
left_arm,right_arm=arms[0],arms[-1]

# A deliberately character-friendly in-place walk: small stride, tiny body bob, opposing arm swing.
...
base_pelvis=pp.location.copy()
# frames: contact L, passing, contact R, passing, loop
poses=[
 (1,  -7,  7,  4, -4, 0.0,  0.7),
 (7,   0,  0,  0,  0, 0.018,-0.3),
 (13,  7, -7, -4,  4, 0.0, -0.7),
 (19,  0,  0,  0,  0, 0.018, 0.3),
 (24, -7,  7,  4, -4, 0.0,  0.7),
]
for f,ll,rr,la,ra,bob,sway in poses:
    pl.rotation_euler.x=math.radians(ll); pr.rotation_euler.x=math.radians(rr)
    pal.rotation_euler.x=math.radians(la); par.rotation_euler.x=math.radians(ra)
    pp.location=base_pelvis+Vector((0,0,bob))
    ps.rotation_euler.z=math.radians(sway)
    for pb,path in ((pl,'rotation_euler'),(pr,'rotation_euler'),(pal,'rotation_euler'),(par,'rotation_euler'),(pp,'location'),(ps,'rotation_euler')):
...
# Studio front-three-quarter render, five review frames.
...
scene.render.resolution_x=scene.render.resolution_y=512; scene.render.resolution_percentage=100; scene.render.image_settings.file_format='PNG'
if scene.world is None: scene.world=bpy.data.worlds.new('WalkWorld')
scene.world.color=(.035,.035,.045)
camloc=centre+Vector((extent*1.5,-extent*3.6,size.z*.10)); bpy.ops.object.camera_add(location=camloc); cam=bpy.context.object; cam.data.type='ORTHO'; cam.data.ortho_scale=extent*1.45; cam.rotation_euler=(centre-cam.location).to_track_quat('-Z','Y').to_euler(); scene.camera=cam
...
for f in (1,7,13,19,24): scene.frame_set(f); scene.render.filepath=str(OUTPUT/f'walk_{f:02d}.png'); bpy.ops.render.render(write_still=True)
...
report={'source':str(model.relative_to(ROOT)),'roles':{'pelvis':pelvis.name,'spine':spine_root.name,'left_leg':left_leg.name,'right_leg':right_leg.name,'left_arm':left_arm.name,'right_arm':right_arm.name},'animation':'Orchid_Prototype_Walk','frames':[1,24],'fps':24,'max_stride_degrees':7,'max_arm_swing_degrees':4,'pelvis_bob':0.018,'principle':'Restrained in-place locomotion authored directly on the Meshy Smart Rig without canned humanoid animation.'}
(OUTPUT/'report.json').write_text(json.dumps(report,indent=2)); bpy.ops.wm.save_as_mainfile(filepath=str(OUTPUT/'prototype-walk.blend')); print(json.dumps(report,indent=2))
