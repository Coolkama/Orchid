...
# 3/4 view retained because it reveals forward/back displacement as well as vertical/lateral movement.
...
# Diagnose only the confirmed right upper-arm candidate first. Each axis gets start/mid/end snapshots
# at 0, +20 and +40 degrees so visual displacement and long-axis roll are easy to distinguish.
bone='Bone_022'; axis_index={'X':0,'Y':1,'Z':2}
...
# Also save bone rest vectors/matrices so the visual result can be reconciled with the rig mathematically.
...
report={
 ...
 'purpose':'three-frame visual classification of Bone_022 local X/Y/Z before rebuilding target-driven arm control'
}
