...
# Blender 4.0 runner: use the legacy blend_method when present; otherwise opaque wire still provides the geometry reference.
...
def set_camera(view):
 loc={'front':cen+Vector((0,-ext*4,0)),'side':cen+Vector((ext*4,0,0)),'threequarter':cen+Vector((ext*2.4,-ext*3.2,ext*.35))}[view];cam.location=loc;cam.rotation_euler=(cen-loc).to_track_quat('-Z','Y').to_euler()
...
for bone in ['Bone_022','Bone_021']:
 ...
 for axis,idx in axis_index.items():
  report[bone][axis]={}
  for deg in [-40,0,40]:
   reset();arm.pose.bones[bone].rotation_euler[idx]=math.radians(deg);bpy.context.view_layer.update();report[bone][axis][str(deg)]={'head':list(arm.pose.bones[bone].head),'tail':list(arm.pose.bones[bone].tail)}
