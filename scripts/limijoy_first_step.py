...
# key one exploratory in-place walking cycle using established X=pitch convention
def pose(f, lhip,lth,lknee,lank, rhip,rth,rknee,rank, bodyz=0):
 vals=[(L['hip'],lhip),(L['thigh'],lth),(L['knee'],lknee),(L['ankle'],lank),(R['hip'],rhip),(R['thigh'],rth),(R['knee'],rknee),(R['ankle'],rank)]
 for n,d in vals:
...
# contact -> passing -> opposite contact -> passing -> loop. Knee flex is intentionally modest for Limijoy's short legs.
pose(1,  8,10, 2,-5, -8,-10,18, 8,0)
pose(9, -2,-4, 4, 2,  4,  7,25,10,0.018)
pose(17,-8,-10,18,8,   8,10, 2,-5,0)
pose(25, 4, 7,25,10, -2,-4, 4, 2,0.018)
pose(33, 8,10, 2,-5, -8,-10,18, 8,0)
for fc in arm.animation_data.action.fcurves:
 for kp in fc.keyframe_points: kp.interpolation='BEZIER'
s.frame_start=1;s.frame_end=32;s.render.fps=24
# ground and camera from skinned mesh bounds
...
cl=cen+Vector((ext*1.35,-ext*3.8,sizez if (sizez:=((mx-mn).z*.05)) else 0));bpy.ops.object.camera_add(location=cl);cam=bpy.context.object;cam.data.type='ORTHO';cam.data.ortho_scale=ext*1.35;cam.rotation_euler=(cen-cam.location).to_track_quat('-Z','Y').to_euler();s.camera=cam
